...
def add_image_text(image, text, img_fraction=0.6):
    ...
    if hasattr(draw, "textsize"):
        ...
        def get_textbbox(text):
            (left, top, right, bottom) = draw.textbbox(xy=(50, 50), text=text, font=font)
...
def render_scenes(source_filepaths, collection_source, satpy_product, area_definition):
    ...
    if satpy_reader is None:
        raise NotImplementedError(f"No satpy reader defined for EUMETSAT collection {collection_source}")
    ...
    for fp_source_zipfile in source_filepaths:
        ...
        if fp_scene_unzipped is None:
            raise RuntimeError(f"Could not find .nat file in unzipped contents of {fp_source_zipfile}")
    ...
    for fp_scene_unzipped in source_filepaths_unzipped:
        ...
        with warnings.catch_warnings():
            ...
            img = get_enhanced_image(local_scn[satpy_product].squeeze(), overlay=None).pil_image()
